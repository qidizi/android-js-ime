...
// 防止idea提示
    if (!window.Vue) window.Vue = new Function();
...
// 还有就是想使用click却由于这个判断出现异常
    function disable_gesture(el) {
        if (el.className.indexOf('disable-gesture') > -1) return true;

        while ((el = el.parentElement)) {
            if (el.className.indexOf('disable-gesture') > -1)
                return true;
        }
    }
    ...
    window.app = new Vue({
        ...
        methods: {
            ...
            "on_touch_start"(ev) {
                ...
                // 如果明确禁用，不走
                if (disable_gesture(target)) return;
